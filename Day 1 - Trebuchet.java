...
"7b";
           ...
           System.out.println("Without words: " +sumCalibration(phrases,false));
           
           System.out.println("With words: " +sumCalibration(phrases,true));
    }
    
    private static Integer sumCalibration(String phrases, boolean words){
        Integer sum = 0;
        String[] lines = phrases.split("\n");
        
        for(String phrase: lines){
            if(words){
                sum += getValueWithWords(phrase);
            }else{
               sum += getValue(phrase); 
            }
    ...
    private static Integer getValue(String phrase){
        
        OptionalInt num1 = phrase.chars()
            .filter(Character::isDigit)
            .map(Character::getNumericValue)
            .findFirst();
            
        OptionalInt num2 = phrase.chars()
            .filter(Character::isDigit)
            .map(Character::getNumericValue)
            .reduce((first, second) -> second); 
        
       //validate if the phrase does not have any numerical value
       return num1.isPresent() && num2.isPresent() ? 
                Integer.valueOf(String.valueOf(num1.getAsInt()) + String.valueOf(num2.getAsInt()))  : 0;
    }
    
    public static int getValueWithWords(String phrase) {
        ...
        for (int i = phrase.length() - 1; i >= 0; i--) {
            if (Character.isDigit(phrase.charAt(i))) {
                num1 = Character.toString(phrase.charAt(i));
            }
            for (String word : wordToDigit.keySet()) {
                if (phrase.startsWith(word, i - word.length() + 1)) {
                    num1 = Integer.toString(wordToDigit.get(word));
                }
            }
        }
        ...
        for (int i = 0; i < phrase.length(); i++) {
            if (Character.isDigit(phrase.charAt(i))) {
                num2 = Character.toString(phrase.charAt(i));
            }
            for (String word : wordToDigit.keySet()) {
                if (phrase.startsWith(word, i)) {
                    num2 = Integer.toString(wordToDigit.get(word));
                }
            }
        }
